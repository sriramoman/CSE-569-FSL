...
def doTraning(x, y, w=np.array((.1, .02, .3, .4, .5, .6, .3, .5, .2)), step=0.001, thresh=0.001):
    grad = np.inf
    ws = np.zeros((9, 0))
    ws = np.hstack((ws, w.reshape(9, 1)))
    ctr = 1
    delta = np.inf
    loss0 = np.inf
    theta = 1
    ...
    while np.abs(delta) > thresh:
        loss, grad = hinge_loss(w+velocity_w*velocity, x, y)
        print loss
        ...
        velocity = velocity_w*velocity - step*nabla
        w = w + velocity
        ws = np.hstack((ws, w.reshape((9, 1))))
        ctr += 1
    print ctr
